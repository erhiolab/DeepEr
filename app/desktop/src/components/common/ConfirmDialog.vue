				...
				<div class="confirm-panel" role="alertdialog" aria-modal="true">
					<header class="confirm-head">
						<div class="confirm-title-row">
							<Icon name="error" :size="18" class="confirm-warn-icon"/>
							<h3 class="confirm-title">{{ title }}</h3>
						</div>
						<button class="confirm-close" @click="close">✕</button>
					</header>
...
.confirm-actions {
	...
	.confirm-btn {
		...
		&.danger {
			border: none;
			color: #fff;
			background-image: linear-gradient(90deg, #fb2c36, #d91f28);

			&:not(:disabled):hover {
				box-shadow: 0 0 1.2rem rgba(251, 44, 54, 0.45);
			}
		}
