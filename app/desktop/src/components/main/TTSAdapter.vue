...
const switchAdapter = async (id: string | null) => {
	...
	const TARGET = id as TTSAdapterId | null
...
const options = computed(() => [
	...
	...TTS_ADAPTERS.map(a => ({id: a.id, label: a.label, description: a.description})),
])
</script>

<template>
	<section class="page-tts">
		...
		<div class="adapter-panel">
			<template v-if="activeAdapter === 'gpt-sovits'">
				<GptSoVITS/>
			</template>
